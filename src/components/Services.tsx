            ...
            <Card key={index} className={`bg-gradient-to-br ${service.gradient} hover:shadow-xl transition-all duration-300 hover:-translate-y-2 ${service.border} border-2`}>
              <CardHeader className="text-center pb-4">
                <div className="flex justify-center mb-4">
                  <div className="p-4 bg-gray-700/80 backdrop-blur-sm rounded-full shadow-lg">
                    {service.icon}
                  </div>
                </div>
                <CardTitle className="text-lg sm:text-xl text-white font-bold">{service.title}</CardTitle>
              </CardHeader>
              <CardContent className="text-center px-4 sm:px-6">
                ...
                <div className="space-y-2 mb-6">
                  {service.features.map((feature, featureIndex) => (
                    <div key={featureIndex} className="flex items-center justify-center text-xs sm:text-sm text-gray-300 font-medium">
                      <div className="w-2 h-2 bg-gradient-to-r from-pink-500 to-purple-500 rounded-full mr-2"></div>
                      {feature}
                    </div>
                  ))}
                </div>
                <Button variant="outline" className="w-full border-2 border-purple-500 text-purple-400 hover:bg-gradient-to-r hover:from-pink-500 hover:to-purple-600 hover:text-white hover:border-transparent transition-all duration-300 text-sm sm:text-base font-semibold">
                  Learn More
                </Button>
              </CardContent>
            </Card>
